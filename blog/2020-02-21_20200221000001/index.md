...
初期値を持つ`Vec<T>`を作成するのがより一般的で、 Rustは`vec!`マクロを提供する

```rust
let v = vec![1, 2, 3];
```

初期値に`i32`を指定したため、 `Vec<i32>`と推測されるため型注釈はは不要になる
...
列挙型を使用すれば、異なる型を入れることができる (列挙型自体は1つの型)
...
複数の文字列を連結する必要がある場合、 `+`演算子の動作は扱いにくい
(結果がわかりにくいため)
