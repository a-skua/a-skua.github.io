...
2. 指定して段階的に実装します．MVP(Minimum Viable Product; 実用最小限のプロダクト)から始め，フィードバックと経験に優先順位付し追加実装を追加します．
...
コンポーネントモデルは常にリンク時の介入をサポートします．
WASI APIはホストに実装されることが多いですが，Wasmでも実装することができ，それ自体がホストのラッパーかもしれません．
...
実用的な場合，WASIはWASI API自体に互換性の問題がないように努め，WASI libcやツールなどのライブラリを通して互換性を提供します．
